...
from utils import slide_header, new_slide, list_item, COLOR_NOTE, code, finish_slides, new_slides
...
that can degrade performance of your programs""", style={"bold": True, "size": 40})

    slide.box(p_top=40).text("""Jakub Beránek
jakub.beranek@vsb.cz""", style={"bold": False, "size": 30})
...
def hw_complexity(slides: Slides):
    slide = new_slide(slides)
    content = slide_header(slide, "Why should we care?")
    list_wrapper = content.box()
    list_item(list_wrapper).text("We write code for the C++ abstract machine")
    list_item(list_wrapper, show="next+").text("Intel CPUs fulfill the contract of this abstract machine")
    list_item(list_wrapper, level=1, show="next+").text("But inside they can do whatever they want")
    list_item(list_wrapper, show="next+").text("Understanding CPU trade-offs can get us more performance")
    ...
    code(content.box(), """void foo(int* arr, int count)
{
    ...
    content.box(p_top=20).text("How fast are the individual array increments?", style={"size": 40})

    slide = new_slide(slides)
    content = slide_header(slide, "Hardware effects")
    list_wrapper = content.box()
    list_item(list_wrapper).text("Performance effects caused by a specific CPU/memory implementation")
    list_item(list_wrapper, show="next+").text("Demonstrate some CPU/memory trade-off or assumption")
    ...
    content.box().text("Source: karlrupp.net", style={"size": 30})
    ...
    content.box().text("Source: Intel Architectures Optimization Reference Manual", style={"size": 30})
...
https://software.intel.com/sites/default/files/managed/9e/bc/64-ia-32-architectures-optimization-manual.pdf""", style={
        "size": 14,
        "align": "left"
    })
    ...
    list_item(list_wrapper, show="next+", p_top=20).text("Disclaimer #1: Everything will be Intel x86 specific")
    list_item(list_wrapper, show="next+").text("Disclaimer #2: I'm not an expert on this and I may be wrong :-)")

    slide = new_slide(slides)
    slide.box().text("""Let's see some examples...""", style={"bold": True, "size": 40})
...
def outro(slides: Slides):
    ...
    slide = new_slide(slides)
    slide.box().text("Thank you!", style={"bold": True, "size": 60})
    slide.box(p_top=60).text("""For more examples visit:
~tt{github.com/kobzol/hardware-effects}""", style={"size": 44})
    slide.box(p_top=80).text("Jakub Beránek")

    slide.box(p_top=100).text("Slides built with ~tt{github.com/spirali/elsie}", style={"size": 30})


def make_presentation(path: str, backup: bool):
    slides = new_slides(1280, 720)

    slides.update_style("default", font=FONT, size=36, color=COLOR1)  # Default font
    slides.update_style("emph", color=COLOR2)     # Emphasis
    slides.update_style("code", size=32)
    slides.derive_style("default", "bold", bold=True)
    slides.derive_style("default", "notice", color=COLOR_NOTE, bold=True)
